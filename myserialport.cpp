      ...
      settingsDialog(new SerialSettingsDialog)
{
    ...
    connect(settingsDialog, &SerialSettingsDialog::sendCommand, this, &MySerialPort::slotWrite);

    updateSerialSetting();
    setCOM(currentSettings.name, currentSettings.baudRate, currentSettings.dataBits, currentSettings.parity, currentSettings.stopBits);
...
{
    emit sigWrite(byteArray);
    //return write(byteArray.data(), byteArray.size());
}

qint64 MySerialPort::write(const unsigned char *data, qint64 maxSize/* = -1*/)
{
    // change unsigned char type to QByteArray type
    const unsigned char* tempBuf = data;
    QByteArray sendData;

    int count = maxSize;
    while(count --)
    {
        qDebug("*tempBuf %02x",*tempBuf);
        QString str = QString::number(*tempBuf, 16).sprintf("%02x", *tempBuf);
        sendData.append(str);
        tempBuf++;
    }
...
#endif
void MySerialPort::updateSerialSetting(QSerialPort *serial)
{

}

void MySerialPort::printSerialSetting(QSerialPort*serial)
{
...
#if 0
void MySerialPort::slotWrite(const unsigned char *pch, qint64 maxSize)
{
    //printSerialSetting(m_pCom);

    //char array[] = "55 AA 05 01 01 10 60 02";
//                   //" 00 00 00 1E 00 00 00 0A 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0000 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 5F FE";
//    qint64 size = sizeof(array);

//    const unsigned char *temp = pch;
//    while(*temp)
//    {
//        qDebug()<<*temp;
//        temp++;
//    }
    ...
    const unsigned char* tempBuf = pch;
    QByteArray sendData;
...
void MySerialPort::slotWrite(const QByteArray &data)
{
    qDebug() << QDateTime::currentDateTime().toString("HH:mm:ss.zzz") << " write begin\n";
    m_lockWriteLen.lock();
    m_iLen = -1;
...
void MySerialPort::updateSerialSetting()
{
    currentSettings.name = settingsDialog->getCurrentSettings().name;
    currentSettings.baudRate = settingsDialog->getCurrentSettings().baudRate;
    currentSettings.dataBits = settingsDialog->getCurrentSettings().dataBits;
    currentSettings.parity = settingsDialog->getCurrentSettings().parity;
    currentSettings.stopBits = settingsDialog->getCurrentSettings().stopBits;
    currentSettings.flowControl = settingsDialog->getCurrentSettings().flowControl;
    currentSettings.localEchoEnabled = settingsDialog->getCurrentSettings().localEchoEnabled;
}
...
void MySerialPort::slotDataReady()
{
    ...
    if(getEchoFlag())
    {
        ...
        qDebug()<<"m_strInBuffer"<<m_strInBuffer;
        if(m_bEchoRegExp)
        {
...
//#endif
    ...
    {
        QByteArray responseData = pCom->readAll();
        while (pCom->waitForReadyRead(300))
        {
            responseData += pCom->readAll();
        }

        QString response(responseData);
        qDebug()<<"Response Data (QString type):"<<response;
        qDebug()<<"Response Data (QByteArray type):"<<responseData;

        emit sigResponseData(response);

        //emit sigDataReady(pCom->readAll()); //原来的代码
    }
}
